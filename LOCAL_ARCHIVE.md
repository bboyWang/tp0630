...
## 2026-07-31（同日第四次）— 已精修样本精确盘点 + PPT 修正

### 1. 已精修样本盘点（权威数 = 225）
- 定义（dataset.py is_human）：`chip_(\d+) <= 377` 或 在 ready.txt 批次中。
- 精确统计（tiff+json 均存在的 1503 个数据集样本内）：
  - chip≤377 区间：101 个（93 仅区间 + 8 与批次重叠）
  - ready.txt 批次：132 个（139 中 7 个缺 json、其中 1 个连 tiff 都没有，未进数据集）
  - **合计去重 = 225**；pool = 1278；v9 划分 train147+val48+test30=225 完全吻合
- 用户"0-377=378"的认知有出入：数据集 chip_id 不连续，仅 116 个 chip≤377 有 tiff，其中 101 个带标注 json。
- 产出 `fine_annotated_samples.txt`：225 个样本完整清单（分 chip≤377 区间 + ready.txt 批次两组）。

### 2. 误导表述修正（"仅 147 已标注" → "225 已精修样本"）
- PPT（gen_ppt.js 重新生成）3 处：S2 数据不平衡 bullet、S2 statCard"147 已标注样本"→"225 已精修样本"、S10"147 标注 vs 1278 未标注"。
- V10_PLAN.md 2 处：1.5 数据质量批次行重写为 225 明细、2.2 学术定位"147 人标"→"225 已精修"。
- 重新生成 PPT + PDF 渲染 QA：改动全部生效、0 溢出、0 越界。
